...
public:
  ...
  bool reduce(int N, const BSets &bs, const TTPTuple &theTuple,
              vector<Word> &gens, int sec, ostream &out, TTPTuple &red_T,
              const Word &z);
...
private:												
  ...
  void tryNode(int N, NODE cur, vector<Word> &gens, const set<NODE> &checkedElements, set<NODE> &uncheckedElements);
  bool process_conjugates(int N, NODE cur, const vector<Word> &gens, const set<NODE> &checkedElements, set<NODE> &uncheckedElements);
