...
TEST(HashedReduce, RemoveDuplicatesTest) {
  std::hash<LongInteger> hasher;
  std::cout << hasher(LongInteger(1)) << std::endl;
  std::cout << hasher(LongInteger(1)) << std::endl;
  std::cout << hasher(LongInteger(1)) << std::endl;
  std::cout << hasher(LongInteger(1)) << std::endl;
  const size_t REPEAT = 10000;
  constexpr size_t RANK = 3;
  const size_t ENDOMORPHISMS_NUMBER = 1;

  typedef TVertexHashAlgorithms<hashers::ImageLengthHash//, hashers::SinglePowerHash
//      , hashers::PermutationHash<Permutation16>
  ...
  while (++seed <= REPEAT) {
    ...
    std::cout <<  "check" << VertexHashAlgorithms::get_vertex_hash(image).get_std_hash() << " "
              <<VertexHashAlgorithms::get_vertex_hash(image).get_std_hash() << std::endl;
