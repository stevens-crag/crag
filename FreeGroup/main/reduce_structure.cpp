...
Vertex reduce(const Vertex& vertex,
  ...
  if (!left && !right) {
    ...
    for (auto& reduction : *last_reductions) {
      if (VertexHashAlgorithms::get_subvertex_hash(first, 0, reduction.first, hash_cache) == VertexHashAlgorithms::get_subvertex_hash(second, 0, reduction.first, hash_cache)) {
    ...
    if (!success) {
      cancellation_length = VertexHashAlgorithms::get_cancellation_length(result, hash_cache);
