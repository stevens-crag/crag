...
int main() {
  ...
  const std::string walnut_file_name("walnut.txt");
  ...
  for (size_t e = init_seed; e < init_seed + experiments_count; ++e) {
    ...
    std::ofstream(walnut_file_name, std::ios::app) << "=================================================" << endl
                                                   << "Experiment #" << e << " started" << std::endl;
    ...
    for (auto attempt = 0u; (attempt < attempt_number) && !success; attempt++) {
    ...
    ofstream OF(walnut_file_name, ios::app);

    if (success) {
      success_count++;
      OF << "Experiment #" << e << " is successful" << std::endl;
      std::cout << "Experiment #" << e << " is successful" << std::endl;
    } else {
      OF << "Experiment #" << e << " failed" << std::endl;
    ...
    OF << "   Time = " << spent_time << std::endl;

    std::cout << "Success: " << success_count << " out of " << (e + 1 - init_seed) << std::endl;
    OF << "Success: " << success_count << " out of " << (e + 1 - init_seed) << std::endl;
  }
